// import { Component } from '@angular/core';
// import { SearchFieldsComponent } from "../../../../../@aava/play-core/src/lib/components/search-fields/search-fields.component";
// import { SearchSuggestion } from '../../../../../@aava/play-core/src/lib/components/search-fields/search-fields.component';

// @Component({
//   selector: 'app-app-searchfields',
//   imports: [SearchFieldsComponent],
//   templateUrl: './app-searchfields.component.html',
//   styleUrl: './app-searchfields.component.scss'
// })
// export class AppSearchfieldsComponent {
//   suggestions: SearchSuggestion[] = [
//     {
//       name: 'Dashboard',
//       logo: 'data:image/png;base64,'
//     },
//     {
//       name: 'Analytics',
//       logo: ''
//     },
//     {
//       name: 'Recent Searches',
//       logo: null
//     },
//     {
//       name: 'Popular Items',
//       logo: null
//     },
//     {
//       name: 'Annual Report',
//       creator: 'John Doe',
//       logo: ''
//     },
//     {
//       name: 'Product Roadmap',
//       creator: 'Jane Smith',
//       logo: ''
//     },
//     {
//       name: 'Dashboard',
//       creator: 'Jane Smith',
//       logo: "data:image/png;base64,"
//     },
//     {
//       name: 'Dashboard'
//     }
//   ];

//   onSuggestionSelected(suggestion: SearchSuggestion) {
//     console.log('Selected suggestion:', suggestion);
//   }
// }


// import { Component } from '@angular/core';
// import { SearchFieldsComponent  } from "../../../../../@aava/play-core/src/lib/components/search-fields/search-fields.component";


// @Component({
//   selector: 'app-app-searchfields',
//   imports: [SearchFieldsComponent],
//   templateUrl: './app-searchfields.component.html',
//   styleUrl: './app-searchfields.component.scss'
// })
// export class AppSearchfieldsComponent {


//   allItems = [
//     'Dover CPQ Demo',
//     'Product Launch',
//     'Sales Report',
//     'Meeting Notes',
//     'Customer Feedback'
//   ];

//   filteredItems = [...this.allItems];

//   handleSearch(query: string) {
//     const lowerQuery = query.toLowerCase();
//     this.filteredItems = this.allItems.filter(item =>
//       item.toLowerCase().includes(lowerQuery)
//     );
//   }
// }


// import { Component } from '@angular/core';
// import { SearchFieldsComponent } from "../../../../../@aava/play-core/src/lib/components/search-fields/search-fields.component";
// import { CommonModule } from '@angular/common';

// @Component({
//   selector: 'app-app-searchfields',
//   standalone: true,
//   imports: [SearchFieldsComponent, CommonModule],
//   templateUrl: './app-searchfields.component.html',
//   styleUrls: ['./app-searchfields.component.scss']
// })
// export class AppSearchfieldsComponent {
//   allItems = [
//     'Dover CPQ Demo',
//     'Product Launch',
//     'Sales Report',
//     'Meeting Notes',
//     'Customer Feedback'
//   ];

//   filteredItems = [...this.allItems];

//   handleSearch(query: string) {
//     const lowerQuery = query.toLowerCase();
//     this.filteredItems = this.allItems.filter(item =>
//       item.toLowerCase().includes(lowerQuery)
//     );
//   }
// }
import {
  Component,
  ElementRef,
  HostListener,
  ViewChild,
  ViewEncapsulation
} from '@angular/core';
import { CommonModule } from '@angular/common';
import { SearchFieldsComponent } from '../../../../../@aava/play-core/src/lib/components/search-fields/search-fields.component';
import { IconsComponent } from '../../../../../@aava/play-core/src/lib/components/icons/icons.component';

@Component({
  selector: 'app-app-searchfields',
  standalone: true,
  imports: [SearchFieldsComponent, CommonModule, IconsComponent],
  templateUrl: './app-searchfields.component.html',
  styleUrls: ['./app-searchfields.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class AppSearchfieldsComponent {
  @ViewChild('codeBlock') codeBlock!: ElementRef;

  sections = [
    {
      title: 'Basic Usage',
      description:
        'Demonstrates the basic functionality of the search component with a list of items.',
      showCode: false
    }
  ];

  apiProps = [
    {
      name: 'placeholder',
      type: 'string',
      default: "'Search...'",
      description: 'The placeholder text for the search input.'
    }
  ];

  events = [
    {
      name: 'search',
      type: 'EventEmitter<string>',
      description: 'Event emitted when the search query changes.'
    }
  ];

  allItems = [
    'Dover CPQ Demo',
    'Product Launch',
    'Sales Report',
    'Meeting Notes',
    'Customer Feedback'
  ];

  filteredItems = [...this.allItems];

  toggleSection(index: number): void {
    this.sections.forEach((section, i) => {
      section.showCode = i === index ? !section.showCode : false;
    });
  }

  toggleCodeVisibility(index: number, event: MouseEvent): void {
    event.stopPropagation();
    this.sections[index].showCode = !this.sections[index].showCode;
  }

  @HostListener('document:click', ['$event'])
  clickOutside(event: MouseEvent) {
    if (this.codeBlock && !this.codeBlock.nativeElement.contains(event.target)) {
      this.sections.forEach(section => (section.showCode = false));
    }
  }

  // ✅ Renamed this method to match what HTML expects
  getExampleCode(sectionTitle: string): string {
    const examples: Record<string, string> = {
      'basic usage': `
import { Component } from '@angular/core';
import { SearchFieldsComponent } from '@awe/@aava/play-core';

@Component({
  selector: 'app-basic-search',
  standalone: true,
  imports: [SearchFieldsComponent, CommonModule],
  template: \`
    <div class="search-demo-container">
      <awe-search-fields
        [placeholder]="'Search...'"
        (search)="handleSearch($event)">
      </awe-search-fields>
      <div class="results">
        <div class="result-item" *ngFor="let item of filteredItems">
          {{ item }}
        </div>
      </div>
    </div>
  \`,
  styles: [\`
    .search-demo-container {
      max-width: 400px;
      margin: auto;
    }
    .results {
      margin-top: 20px;
    }
    .result-item {
      padding: 10px;
      border-bottom: 1px solid #ccc;
    }
  \`]
})
export class BasicSearchComponent {
  allItems = [
    'Dover CPQ Demo',
    'Product Launch',
    'Sales Report',
    'Meeting Notes',
    'Customer Feedback'
  ];

  filteredItems = [...this.allItems];

  handleSearch(query: string) {
    const lowerQuery = query.toLowerCase();
    this.filteredItems = this.allItems.filter(item =>
      item.toLowerCase().includes(lowerQuery)
    );
  }
}
      `
    };

    return examples[sectionTitle.toLowerCase()] || '';
  }

  handleSearch(query: string) {
    const lowerQuery = query.toLowerCase();
    this.filteredItems = this.allItems.filter(item =>
      item.toLowerCase().includes(lowerQuery)
    );
  }

  copyCode(sectionTitle: string) {
    const code = this.getExampleCode(sectionTitle.toLowerCase());
    navigator.clipboard.writeText(code).then(() => {
      console.log('Code copied to clipboard!');
    });
  }
}
